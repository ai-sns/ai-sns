...
from db.DBFactory import query_AgentTask, query_AgentTask_Search_Content, query_AgentTask_Content, \
    ...
    query_AgentTask_ById, query_AgentTask_ByLabel, query_map_tools, query_single_map_tool, delete_map_tool, update_map_tool, query_tool_list, query_single_tool
...
from aimaptooldialog import ConfigDialog as AiMapToolDialog


class MapToolList(QTreeWidget):
    """TaskList implements the view in a Tree of the Roster"""
    rename_signal = pyqtSignal(object)
    label_signal = pyqtSignal(object)

    def __init__(self, parent, agent):
        super(MapToolList, self).__init__(parent)
        ...
        self.setHeaderLabel(lt("Tool List", "工具列表"))  # 需要设置此处的值，否则缺省值为1
    ...
    def load_pop_menu(self):
        ...
        self.re_fresh_action = QAction(QIcon("images/refresh.png"), "刷新", self)
        self.re_fresh_action.triggered.connect(self.refresh)
        self.menu.addAction(self.re_fresh_action)
    ...
    # --> 加载 数据
    def load_data(self):
        # self.tasklist = query_map_tools()
        self.tasklist = query_tool_list()
        for record in self.tasklist:
            self.addItem(record.name.replace("\n", ""), int(record.id), False, 0)
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Delete:
            ...
            super(MapToolList, self).keyPressEvent(event)
    ...
    def start_task(self):
        ...
        if id_value:
            ...
            #                                  QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            # if reply == QMessageBox.Yes:
            #     update_map_tool(id_value, status=1)
            #     record = query_single_map_tool(id=id_value)
        ...
        if id_value:
            if item:
                reply = QMessageBox.question(self, '确定',
                                             ...
                                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if reply == QMessageBox.Yes:
                update_map_tool(id_value, status=0)
        ...
        if id_value:
            if item:
                reply = QMessageBox.question(self, '确定',
                                             ...
                                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if reply == QMessageBox.Yes:
                update_map_tool(id_value, status=2)
                self.mainwindow.map_message_box.stop_task()
                item_icon = QIcon()
                item.setIcon(0, item_icon)
    ...
    def re_plan_task_and_start(self):
        ...
        if id_value:
            ...
            #                                  QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            # if reply == QMessageBox.Yes:
            #     update_map_tool(id_value, status=0,sub_task_list="",current_sub_task="")
            #     self.mainwindow.map_message_box.stop_task()
            #     item_icon = QIcon()
            #     item.setIcon(0, item_icon)
            #     self.start_task()

            update_map_tool(id_value, status=1, sub_task_list="", current_sub_task="")
    ...
    def re_excecute_plan(self):
        ...
        if id_value:
            ...
            update_map_tool(id_value, status=1, current_sub_task="")
            self.mainwindow.map_message_box.re_init_task()
            self.mainwindow.map_message_box.stop_task()
            item_icon = QIcon()
            item.setIcon(0, item_icon)
    ...
    def on_itemDoubleClicked(self, item, column):
        ...
        # task_record = query_single_map_tool(id=id_value)

        task_record = query_single_tool(id=str(id_value))
        ai_map_tool_dialog = AiMapToolDialog(self, task_record)
        ai_map_tool_dialog.exec()
