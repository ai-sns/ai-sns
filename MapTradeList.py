...
from db.DBFactory import query_AgentTask, query_AgentTask_Search_Content, query_AgentTask_Content, \
    ...
    query_AgentTask_ById, query_AgentTask_ByLabel,query_map_trades,query_single_map_trade,delete_map_trade,update_map_trade,query_single_map_trade
...
from aimaptradedialog import ConfigDialog as AiMapTradeDialog

class MapTradeList(QTreeWidget):
    ...
    def __init__(self, parent, agent):
        ...
        super(MapTradeList, self).__init__(parent)
        ...
        self.setHeaderLabel(lt("Trade List","交易列表"))  # 需要设置此处的值，否则缺省值为1
    ...
    def load_data(self):
        ...
        self.tradelist = query_map_trades()
        for record in self.tradelist:
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key.Key_Delete:
            ...
            super(MapTradeList, self).keyPressEvent(event)
    ...
    def start_task(self):
        ...
        if id_value:
            ...
            #                                  QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            # if reply == QMessageBox.Yes:
            #     update_map_trade(id_value, status=1)
            #     record = query_single_map_trade(id=id_value)
            #     self.mainwindow.map_message_box.set_current_trade_record(record)
        ...
        if id_value:
            if item:
                reply = QMessageBox.question(self, '确定',
                                             ...
                                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if reply == QMessageBox.Yes:
                update_map_trade(id_value, status=0)
        ...
        if id_value:
            if item:
                reply = QMessageBox.question(self, '确定',
                                             ...
                                             QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            if reply == QMessageBox.Yes:
                update_map_trade(id_value, status=2)
                self.mainwindow.map_message_box.stop_task()
                item_icon = QIcon()
                item.setIcon(0, item_icon)
    ...
    def re_plan_task_and_start(self):
        ...
        if id_value:
            ...
            #                                  QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
            # if reply == QMessageBox.Yes:
            #     update_map_trade(id_value, status=0,sub_task_list="",current_sub_task="")
            #     self.mainwindow.map_message_box.stop_task()
            #     item_icon = QIcon()
            #     item.setIcon(0, item_icon)
            #     self.start_task()

                    update_map_trade(id_value, status=1, sub_task_list="", current_sub_task="")
    ...
    def re_excecute_plan(self):
        ...
        if id_value:
            ...
            #     self.start_task()
                    ...
                    update_map_trade(id_value, status=1, current_sub_task="")
                    self.mainwindow.map_message_box.re_init_task()
                    self.mainwindow.map_message_box.stop_task()
                    item_icon = QIcon()
                    item.setIcon(0, item_icon)
    ...
    def on_itemDoubleClicked(self, item, column):
        ...
        trade_record = query_single_map_trade(id=id_value)

        ai_map_trade_dialog = AiMapTradeDialog(self,trade_record)
        ai_map_trade_dialog.configured.connect(self.mainwindow.on_configured_ai_map)
        ai_map_trade_dialog.exec()
