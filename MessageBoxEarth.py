...
from db.DBFactory import (query_AgentCfg, add_AIChatMessages, get_prompt_by_title, query_function_mng,
                          add_function_mng, add_AgentTask, add_map_task, query_map_tasks, query_single_map_task,
                          update_map_task, delete_map_task, update_map_task, add_map_visit, get_key_value, add_map_tool,
...
from util import (generate_random_id, add_msg_to_message_window, get_myai_send_msg_title_formatted,
                  add_msg_to_message_windowv3, get_user_ask_msg_title_formatted, get_user_ask_msg_content_formatted,
                  get_agent_reply_msg_title_formatted, get_agent_reply_msg_content_formatted, toggle_msg_loading_status,
                  add_agent_reply_msg_to_message_window, add_msg_to_message_window_with_markdown_and_highlight, add_attachment_to_message_window,
                  image_to_base64, generate_img_tag, add_msg_to_message_window_with_markdown_and_highlightv2,add_memory_list)
from i18n import lt
...
from prompts_sns import PromptDialog, PromptManager
...
from ui.ui_ChartCanvas import RadarChartCanvas,BarChartCanvas
# 配置日志记录
...
子任务列表，当前目标,当前目标和主任务相关性及评分，请按如下json格式输出:
{
    ...
    #3.执行具体子任务
    def ask_agent_instruction_to_process_activitybak(self, ask_content):
        self.show_status_on_map("thinking")
        if not self.started_flag:
            return

        role_prompt = get_prompt_by_title("__instruction_to_process_activity_role__")
        ...
        full_ask_content = self.compose_full_ask_content(task_description, ability_list, question_to_llm)
