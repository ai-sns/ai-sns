            ...
            with self._client.messages.stream(**anthropic_params) as stream:
                for event in stream:
                    if event.type == "input_json":
                        # print(f"input delta: {repr(event.partial_json)}")
                        # print(f"input snapshot: {event.snapshot}")
                        print("")

                    elif event.type == "text":
                        chunk =event.text
                        if chunk is not None:
                            iostream.print(chunk, end="", flush=True)

                    elif event.type == "message_start":
                        content_id = event.message.id

                    elif event.type == "content_block_start":
                        print("content_block_stop：", event.content_block.type)
                        if event.content_block.type == "text":
                            content = event.content_block.text
                            print("content:", content)
                        elif event.content_block.type == "tool_use":
                            function_id = event.content_block.id
                        ...
                        if event.content_block.type == "text":
                            ...
                            print("functioname", function_name)
                            function_input = event.content_block.input
                            print("functioninput", function_input)
            ...
            if function_name:
                ...
                tool_calls.append(
                    ChatCompletionMessageToolCall(
                        id=function_id,
                        function={"name": function_name, "arguments": json.dumps(function_input)},
                        type="function",
                    )
                )
