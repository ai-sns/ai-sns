...
│   InitializationWizard / SNSAvatarDialog / SNSSidebar / map.html         │
│                              │ HTTP + WebSocket                          │
│                              ▼                                           │
...
- 用户在 SNSAvatarDialog 修改 a2a_config 后，`service_async.update_ai_chat_config()` 调度热重载：
...
优先级：
1. **inline**：`aisns_cfg.memo.a2a_config.agent_card`（前端 `SNSAvatarDialog` 的"Agent Card" 表单写入，`@c:\dev\agi-ev\ai-sns-el\aisns_frontend\renderer\js\modules\sns\SNSAvatarDialog.js:1281-1336`）。
...
`SNSAvatarDialog.js` "XMPP" Tab 内：
- 字段：`xmppAccount / xmppPassword / a2aCardName / Description / Version / Provider Org / Provider URL`（`@c:\dev\agi-ev\ai-sns-el\aisns_frontend\renderer\js\modules\sns\SNSAvatarDialog.js:279-326`）。
- 命令列表通过 `_loadMergedA2ACommands` 拉取后端合并结果：

```@c:\dev\agi-ev\ai-sns-el\aisns_frontend\renderer\js\modules\sns\SNSAvatarDialog.js:1057-1075
...
保存时 `_collectA2AConfig`（`@c:\dev\agi-ev\ai-sns-el\aisns_frontend\renderer\js\modules\sns\SNSAvatarDialog.js:1281-1336`）打包 `{agent_card, adhoc_commands}` 写回 `aisns_cfg.memo.a2a_config`；`service_async.update_ai_chat_config` 检测到变更后调度 `reload_a2a()`（不掉线）或者全量 `restart()`（凭据变更）。
...
```
SNSAvatarDialog 保存 → PUT /api/sns/ai-chat-config (a2a_config=…)
...
- **前后端**：前端 InitializationWizard 决定 XMPP 凭据、SNSAvatarDialog 决定 a2a_config / 命令开关，后端通过 `reload_a2a` 实现无缝热更新；地图通过 WebSocket 实时呈现 XMPP 消息。
