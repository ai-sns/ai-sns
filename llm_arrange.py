...
from db.DBFactory import update_web_mng,delete_web_mng

class FreezeTableDialog(QDialog):
    def __init__(self, model,parent=None,type_str="LLM"):
    ...
    def deleteSelectedRows(self):
        ...
        if selected_indexes:
            ...
            if confirmation == QMessageBox.Yes:
                ...
                delete_web_mng(web_id=agent_user_id)

                rows_to_delete = [index.row() for index in selected_indexes]
                for row in reversed(rows_to_delete):
                    self.model.removeRow(row)
                self.model.layoutChanged.emit()
                self.app.refresh_toolbox_llm_web(self.type_str)
    ...
    def update_agent_index(self):
        for row in range(self.model.rowCount()):
            web_id = self.model.index(row, 1).data()
            update_web_mng(web_id,position=row)
