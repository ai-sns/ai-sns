...
class MCPClientController:
    def __init__(self, client_script_path: str = "client_subprocess.py"):
        ...
        #     stdin=subprocess.PIPE,  # 允许向进程的标准输入写入数据
        #     stdout=subprocess.PIPE,  # 可选：允许捕获标准输出
        #     stderr=subprocess.PIPE,  # 可选：允许捕获标准错误
        #     text=True  # 以文本模式处理输入和输出
        # )
        #
        # # 示例：向子进程的标准输入写入数据
        ...
        # 重要说明：Popen本身是非阻塞的，但后续操作可能阻塞
        # communicate()是阻塞调用，会等待进程结束
        try:
            # 写入输入数据（非阻塞，但管道缓冲区满时会阻塞）
            input_data = '{"command": "connect", "args": {"path":"server.py"}}\n'
            process.stdin.write(input_data)
            process.stdin.flush()  # 确保数据立即发送

            # 更安全的替代方案：使用communicate()处理完整交互
            # 注意：communicate()会关闭管道并等待进程结束
            ...
            while True:
                ...
                # 非阻塞检查输出
                rlist, _, _ = select([process.stdout, process.stderr], [], [], timeout)
                for stream in rlist:
