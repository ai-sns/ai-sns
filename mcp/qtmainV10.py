...
class MCPClientWorker:
    ...
    def __init__(self, server_script_path: str, command_queue: multiprocessing.Queue, response_queue: multiprocessing.Queue):
        """
        初始化工作器
...
def worker_process_entry(server_script_path: str, command_queue: multiprocessing.Queue, response_queue: multiprocessing.Queue):
    """工作进程入口点"""
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # 服务器脚本输入框
        self.server_script_input = QLineEdit("server.py")
        self.server_script_input.setPlaceholderText("输入服务器脚本路径...")
        self.server_script_input.setFixedHeight(40)
        # 查询输入框
        ...
        layout.addWidget(self.server_script_input)
    ...
    def on_connect_clicked(self):
        """连接按钮点击处理"""
        server_script = self.server_script_input.text().strip()
        if not server_script:
            self.status_display.append("错误: 请先输入服务器脚本路径")
            return

        self.status_display.append(f"正在连接服务器: {server_script}...")
        self.connect_btn.setEnabled(False)

        # 在这里创建并启动进程管理器
        self.process_manager.start(server_script)
    ...
    def on_worker_shutdown(self):
        """工作线程关闭完成时的处理"""
        self.status_display.append("工作进程已安全关闭")
    ...
    def closeEvent(self, event):
        ...
        if self._closing:
            ...
            return
        self._closing = True
        self.status_display.append("正在关闭工作进程...")
