...
class MainWindow(QMainWindow):
    ...
    def on_connect_clicked(self):
        """连接按钮点击处理"""
        server_script_path = self.server_path_input.text().strip()
        if not server_script_path:
            self.status_display.append("错误: 请输入服务器脚本路径")
            return
        self.status_display.append(f"正在连接服务器: {server_script_path}...")
        self.connect_btn.setEnabled(False)
        self.process_manager.start(server_script_path)
