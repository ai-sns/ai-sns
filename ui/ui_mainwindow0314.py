...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.actionShow_ai_homepage = QtGui.QAction(MainWindow)
        ...
        self.actionShow_human_homepage = QtGui.QAction(MainWindow)
        ...
        self.actionShow_km_homepage = QtGui.QAction(MainWindow)
        ...
        self.actionShow_plugin_homepage = QtGui.QAction(MainWindow)
    ...
    def createToolBoxUnit_AgentChat(self, agent, pos=-1):
        ...
        tabWidget.addTab(labelList, lt("Tags","标签列表"))  # -->
    ...
    def createToolBoxUnit_AiChat_earth(self, agent, pos=-1):
        ...
        radar_categories = [f'{lt("IQ","智力")}:5', f'{lt("Growth","成长")}:4', f'{lt("Tech","技能")}:5', f'{lt("KM","知识")}:2', f'{lt("Speed","速度")}:1']

        # 定义柱状图数据
        bar_indicators = [f'{lt("Wealth","财富")}:100K', f'{lt("Credit","诚信")}:90', f'{lt("Level","等级")}1']  # 使用中文标签
        bar_values = [5, 6, 8]
        ...
        config_button=self.create_ai_cfg_button_earth(lt("Setting＆Help","设置&帮助"), agent, 0)



        mission_button =self.create_ai_task_button_earth(lt("New Task","指派任务"), agent, 0)
        ...
        if pos == -1:
            self.toolBox_AiChat.addItem(itemWidget, QIcon('images/earth.png'),lt("Explore the Earth", "漫游地球", "漫游地球") + "-" + agent.nickname)
        else:
            self.toolBox_AiChat.insertItem(self.toolBox_AiChat.count() - 1, itemWidget,
                                           QIcon('images/earth.png'), lt("Explore the Earth", "漫游地球", "漫游地球") + "-" + agent.nickname)
        # textEdit.returnPressed.connect(lambda: buddyList.search(textEdit.text()))
        textEdit.returnPressed.connect(lambda: self.filterItemsBuddyList(textEdit.text()))
    ...
    def createToolBox_AiChat(self):
        ...
        for record in records:
            ...
            # if record.user_id !="1":
            if record.user_id == "JA2024091910112279241":
        ...
        self.toolBox_AiChat.addItem(backgroundWidget, QIcon('images/setting.png'), lt("Account setting","帐号管理"))
    ...
    def createToolBoxUnit_HumanChat(self, agent):
        ...
        # two button 两个按钮
        layout = QGridLayout()
        layout.addWidget(self.create_new_contact_group_button("添加联系人/组", None, 0),
                         0, 0)
        layout.addWidget(self.create_human_cfg_button(lt("Setting","更多设置"), agent, 0), 0,
                         1)
    ...
    def createToolBox_HumanChat(self):
        ...
        self.toolBox_HumanChat.setSizePolicy(QSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Ignored))
    ...
    def createToolBox_KM(self):
        ...
        backgroundLayout = QGridLayout()
        backgroundLayout.addWidget(self.createCellWidgetKMNew(lt("New KB","新增知识库"),
                                                              'images/bookplus.png'), 0, 0)
        backgroundLayout.addWidget(self.createCellWidgetKMMng(lt("KB Management","管理知识库"),
                                                              'images/filecabinet.png'), 0, 1)

        backgroundLayout.addWidget(self.createCellWidgetKVMng(lt("Key-Value","管理键值对"),
        ...
        self.toolBox_KM.addItem(backgroundWidget, QIcon('images/setting.png'), lt("KM Setting","知识库设置"))
    ...
    def createToolBox_Plugin(self):
        ...
        self.layout_tool.addWidget(self.create_install_plugin_local_button(lt("Import","导入本地插件"),
                                                                                'images/add.png'), 1, 0)
        ...
        self.toolBox_Plugin.setMinimumWidth(itemWidget.sizeHint().width())
        self.toolBox_Plugin.addItem(itemWidget, QIcon('images/skill.png'), lt("Skill","已学技能"))
        ...
        self.backgroundLayout.addWidget(self.download_plugin_button(lt("Download", "下载插件")), 0, 0)
        self.backgroundLayout.addWidget(self.publish_plugin_button(lt("Publish", "发布插件")), 0, 1)

        self.backgroundLayout.setRowStretch(row + 1, 10)
        ...
        self.toolBox_Plugin.addItem(backgroundWidget, QIcon('images/market.png'), lt("Plugin Market","插件市场"))
    ...
    def createToolBox_WorkFlow(self):
        ...
        layout_tool.addWidget(self.create_workflow_cfg_button(0),
                              0, 0)

        layout_tool.addWidget(self.create_task_schedule_button(1),
                              0, 1)
    ...
    def createToolBox_Setting(self):
        ...
        self.toolBox_Setting.setSizePolicy(QSizePolicy(QSizePolicy.Policy.Maximum, QSizePolicy.Policy.Ignored))
        self.toolBox_Setting.setMinimumWidth(itemWidget.sizeHint().width())
        self.toolBox_Setting.addItem(itemWidget, QIcon('images/setting.png'), lt("Setting","系统管理"))
    ...
    def createActions(self):
        ...
        self.ai2meAction = QAction(QIcon('images/agent.png'),
                                   "为我处理工作的Ai智能体", self, shortcut="Ctrl+9",
        ...
        self.ai2meAction.setFont(font)
        # self.ai2meAction.setCheckable(False)  # 设置为可切换状态
        ...
        self.ai2aiAction = QAction(
            ...
            triggered=self.ShowAiChatStack, iconText=lt("SNS","AI社交"))

        self.kmAction = QAction(
            QIcon('images/km.png'), lt("KM","知识库"),
            self, shortcut="Ctrl+2", statusTip="知识库",
            triggered=self.ShowKMStack, iconText=lt("KM","知识库"))

        self.pluginAction = QAction(
            QIcon('images/tool.png'), lt("Plugin","插件工具"),
            self, shortcut="Ctrl+3", statusTip="插件工具",
            triggered=self.ShowPluginStack, iconText=lt("Plugin","插件工具"))

        self.chatAction = QAction(
            QIcon('images/humanchat.png'), "人类的社交，比如在线聊天",
            self, shortcut="Ctrl+4", statusTip="人类的社交，比如在线聊天",
            triggered=self.ShowHumanChatStack, iconText="人类社交")

        self.workflowAction = QAction(
            QIcon('images/workflow.png'), lt("WorkFlow","工作流"),
            self, shortcut="Ctrl+5", statusTip="工作流",
            triggered=self.ShowWorkFlowStack, iconText=lt("WorkFlow","工作流"))

        self.webAction = QAction(
            QIcon('images/webresource.png'), lt("Web","网络资源"),
            self, shortcut="Ctrl+6", statusTip=lt("Web","网络资源"),
            triggered=self.ShowWebStack, iconText=lt("Web", "网络资源"))

        self.settingAction = QAction(
            QIcon('images/aisns150.png'), lt("Home","首页"),
            self, shortcut="Ctrl+7", statusTip="系统管理",
            triggered=self.ShowSettingStack, iconText=lt("Home","首页"))

        # self.syshelpAction = QAction(
        #     QIcon('images/help.png'), lt("Help","系统帮助"),
        #     self, shortcut="Ctrl+H", statusTip="系统帮助",
        #     triggered=self.openwebhelp, iconText=lt("Help","系统帮助"))
    ...
    def createToolbars(self):
        ...
        ai_toolbar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
        # ai_toolbar.setFixedHeight(350)
        ai_toolbar.setFixedHeight(600)
        ai_toolbar.setFixedWidth(70)
        self.aiToolBar = self.addToolBar(Qt.ToolBarArea.LeftToolBarArea, ai_toolbar)

        # Create setting Toolbar
        setting_toolbar = QToolBar("Human")

        setting_toolbar.addAction(self.webAction)
        setting_toolbar.addAction(self.settingAction)
        # setting_toolbar.addAction(self.syshelpAction)
        setting_toolbar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
        self.settingToolBar = self.addToolBar(Qt.ToolBarArea.LeftToolBarArea, setting_toolbar)
        ...
        # ai2_toolbar.setToolButtonStyle(Qt.ToolButtonStyle.ToolButtonTextUnderIcon)
        # ai2_toolbar.setFixedHeight(250)
        # self.aiToolBar2 = self.addToolBar(Qt.ToolBarArea.LeftToolBarArea, ai2_toolbar)
    ...
    def open_plugin_market(self):
        self.plugin_home.page().load(QUrl("http://www.ai-sns.org/index_plugin.html"))
    ...
    def create_ai_cfg_button_earth(self, text, agent, diagramType):
        ...
        # 创建按钮并添加图标
        agentcfgbutton = QPushButton(lt("Setting＆Help","设置&帮助"))
    ...
    def createColorMenu(self, slot, defaultColor):
        colors = [Qt.GlobalColor.black, Qt.GlobalColor.white, Qt.red, Qt.blue, Qt.yellow]
